...
def greedy_search(graph, start, goal, coordinates):
    """Recherche Greedy en utilisant la distance en ligne droite comme heuristique."""
    frontier = [(0, start, [start])]  # (heuristique, noeud courant, chemin)
    visited = []

    while frontier:
        # Trier la frontière par heuristique croissante
        frontier.sort(key=lambda x: x[0])
        # Retirer le premier élément de la frontière
        _, current_node, path = frontier.pop(0)

        if current_node == goal:
            return path
        visited.append(current_node)
        for neighbor in graph[current_node]:
            if neighbor not in path and neighbor not in visited:  # Évite les boucles dans le chemin
                heuristic = haversine(coordinates[neighbor], coordinates[goal])
                new_path = path + [neighbor]
                frontier.append((heuristic, neighbor, new_path))
...
if __name__ == "__main__":
    ...
    start_node = 9
    end_node = 44
    path = greedy_search(graph, start_node, end_node, coordinates)
