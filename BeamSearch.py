...
def beam_search(graph, start, goal, coordinates, beam_width):
    """Beam Search avec la distance en ligne droite comme heuristique."""
    frontier = [(0, start, [start])]  # (heuristique, noeud courant, chemin)
    while frontier:
        new_frontier = []
        for _, current_node, path in frontier:
            if current_node == goal:
                return path
            for neighbor in graph[current_node]:
                if neighbor not in path:
                    heuristic = haversine(coordinates[neighbor], coordinates[goal])
                    new_frontier.append((heuristic, neighbor, path + [neighbor]))
        # Gardez seulement les 'beam_width' meilleurs éléments pour la prochaine expansion
        frontier = sorted(new_frontier, key=lambda x: x[0])[:beam_width]
    return None

def draw_path_on_map(path, coordinates):
    """Dessine le chemin trouvé sur une carte Folium."""
    if path:
...
if __name__ == "__main__":
    ...
    beam_width = 4  # Largeur du faisceau
    start_node = 1
    end_node = 54
    path = beam_search(graph, start_node, end_node, coordinates, beam_width)
    execution_time_ms = (time.time() - start_time) * 1000  # Conversion du temps d'exécution en millisecondes
    map = draw_path_on_map(path, coordinates)
    if map:
        map_filename = './maps/beam_search_path_map.html'
        map.save(map_filename)
        print(f"Chemin trouvé du nœud {start_node} au nœud {end_node}:", path)
        print(f"La carte avec le chemin a été enregistrée sous '{map_filename}'.")
    else:
        print("Le chemin n'a pas pu être tracé sur la carte.")

    print(f"Temps d'exécution: {execution_time_ms:.4f} ms")  # Affichage du temps en secondes avec quatre chiffres après la virgule
